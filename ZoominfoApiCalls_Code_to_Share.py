...
def get_contact_data_search_api(page,radius,zipcode,industry,company_name,titles,email,phone,name,contacts_per_site,req_contacts_ind,retry=1):
    ...
    body = {
        ...
        "rpp":50, # results per page , 50 * 30 is 1500
        "revenueMin": 1000000
     };
...
def get_intent_data_search_api(page,radius,zipcode,industry,company_name,retry=1):
    ...
    body = {
        ...
        "signalScoreMin": 80,
        "rpp": 50,
        "findRecommendedContacts": False,
        "sortBy": "signalScore",
        #"revenueMin": 1000000,
        "sortOrder": "desc"
    };

    body["signalStartDate"]="2022-08-21"
...
#def get_prospects_in_zip_code(zips):
    ...
    #cnxn = pyodbc.connect(
        #r'Driver={SQL Server};Server=;Database=;Trusted_Connection=;')
...
def run_customer_matchback(enriched_contacts):
 ...
 # comment out this section if you want to remove bnbill addrees
    ...
    # commenr tiill here uncommeht this : return filter1.copy(deep=True)
...
def get_industry_mha_contacts(industry,req_contacts_ind,titles,zipcode,contacts_per_site=1):
    num_contacts = 0
    num_loops = 30 # max nubler of pages zoominfo api
    loops=0
    page=0
...
def get_industry_intent_contacts(industry, req_contacts_ind, titles, zipcode, contacts_per_site=1):
    ...
    while page < 5:
        ...
        intent_companies = pd.concat([intent_companies, intent_companies_temp]) if len(intent_companies) > 0 and len(
            intent_companies_temp) > 0 \
...
region_num_list = region_metadata_file.Regnum.unique(); # regions in file
...
if loginstatus.status_code == 200:
    ...
    if len(contacts)>0:
        pd.concat(contacts).to_csv('zi_testpull.csv')
